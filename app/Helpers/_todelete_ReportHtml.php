...
class ReportHtml
{
  ...
  public function reportGenerate($data)
  {
    ...
    if ($data['isFooter']) {
      ...
      // $descriptionText = empty($data['description']) ? "\u{200B}" : $data['description'];
      ...
      $row2 = $table3->addRow();
      $row2->addCell(0, $cellRowContinue);
      $row2->addCell(0, $cellRowContinue);
      $row2->addCell(1500)->addText($data['page']);
      $row2->addCell(1500)->addText($data['pages']);
    }
    ...
    $fileName = $randomString . '-' . $timestamp . '.' . 'html'; // Формирование имени файла
