            ...
            @foreach ($responsiblePersons as $index => $person)
                <div class="mb-2 mt-2 flex flex-row items-center gap-4 rounded-md bg-sky-100 p-4 md:flex-row">
                    <div class="md:full basis-4/12">
                        <x-input type="text" class="block w-full" wire:model="responsiblePersons.{{ $index }}.kind" value="{{ $person['kind'] }}" />
                    </div>
                    <div class="md:full basis-4/12">
                        <x-input type="text" class="block w-full" wire:model="responsiblePersons.{{ $index }}.surname" value="{{ $person['surname'] }}" />
                    </div>
                    <div class="md:full basis-4/12">
                        <x-input type="date" class="block w-full" wire:model="responsiblePersons.{{ $index }}.signdate" />
                    </div>
                    <div class="md:full basis-4/12">
{{--                        <x-input type="text" class="block w-full">{{ $person['file_src'] }}</x-input>--}}
            ...
            <table class="w-full table-auto text-left text-sm text-gray-500 rtl:text-right dark:text-gray-400">
                ...
                @foreach ($signsList as $item)
                    <tr class="border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <td class="px-3 py-2">
                            <x-info-button title="Добавить подпись" wire:click.prevent="addToResponsiblePersons('{{ $item['kind'] }}', '{{ $item['surname'] }}', '{{ $item['file_src'] }}')">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
    ...
    <script type="module">
        ...
        async function update(file) {
            ...
            resultElement.innerHTML = "Загрузка файла...";
